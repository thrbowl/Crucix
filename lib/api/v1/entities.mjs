...
import { queryObjects, countByType, getObjectById } from '../../stix/objects.mjs';
...
export default function entitiesRouter({ getPool }) {
  ...
  // POST /entities/summary { q?, date_from?, date_to? } — type distribution under current filters
  router.post('/summary', requireCredits('entity_query', getPool()), async (req, res) => {
    const pool = getPool();
    ...
    if (date_from) { conditions.push(`(data->>'created')::timestamptz >= $${idx++}`); params.push(date_from); }
    if (date_to)   { conditions.push(`(data->>'created')::timestamptz <= $${idx++}`); params.push(date_to); }

    const where = conditions.length > 0 ? 'WHERE ' + conditions.join(' AND ') : '';
    try {
      const result = await pool.query(
        `SELECT type, COUNT(*)::int AS count FROM stix_objects ${where} GROUP BY type ORDER BY count DESC`,
        params,
      );
      const types = result.rows;
      const total = types.reduce((sum, t) => sum + t.count, 0);
      sendApiResponse(res, { types, total }, creditsFromReq(req, CREDIT_COSTS.entity_query));
    } catch (err) {
      console.error('[API v1] entities summary error:', err.message);
      res.status(500).json({ error: 'Failed to query entity summary' });
    }
  });

  // GET /entities/:type?filter=&sort=&page=&limit=  — 列表（1 积分）
  // Use type="_all" to list across all types
  router.get('/:type', requireCredits('entity_query', getPool()), async (req, res) => {
    const pool = getPool();
    if (!pool) return res.status(503).json({ error: 'Database not configured' });

    const { type } = req.params;
    const queryAll = type === '_all';
    if (!queryAll && !VALID_TYPES.has(type)) {
    ...
    try {
      const objects = await queryObjects(pool, {
        type: queryAll ? undefined : type,
        limit, offset, minScore, dateFrom, dateTo,
      });
      sendApiResponse(
        res,
        { type: queryAll ? '_all' : type, page, limit, items: objects, total: objects.length },
